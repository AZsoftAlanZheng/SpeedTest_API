...
if(!fs.existsSync(csvFilePath)) {
    console.error("CSV File Path: " + csvFilePath + " does not exists!!! ");
	process.exit(1);
}
if(filterCarrierName && Carrier.indexOf(filterCarrierName) < 0) {
	console.error("Carrier Name: " + filterCarrierName + " not in array!!! " + Carrier);
	process.exit(1);
}
	    ...
	    if (finishedTask.indexOf(element) > -1) {
			...
			request({
				headers: {
					...
					'X-Droi-Session-Token': 'e135ab2a3a0c74cd'
				},
				...
				method: 'GET'
					...
					if(err) {
						...
						if(!json || !json.hasOwnProperty("Result") || !json.Result.hasOwnProperty("SpeedTest") ) {
							console.error('!json || !json.hasOwnProperty("Result") || !json.Result.hasOwnProperty("SpeedTest")');
							process.exit(1);
						}
