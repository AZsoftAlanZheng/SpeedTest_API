...
var Pitboss, TransactionRunner, addHooks, async, caseless, chai, clone, eventCallback, gavel, headersArrayToObject, logger, os, packageData, path, requestLib, sandboxedLogLibraryPath, sortTransactions, url,
...
headersArrayToObject = function(arr) {
  var i, len, name, obj, ref, value;
  obj = {};
  for (i = 0, len = arr.length; i < len; i++) {
    ref = arr[i], name = ref.name, value = ref.value;
    obj[name] = value;
  }
  return obj;
};

eventCallback = function(reporterError) {
  if (reporterError) {
    return logger.error(reporterError.message);
  }
};
...
TransactionRunner = (function() {
  ...
  TransactionRunner.prototype.runHooksForData = function(hooks, data, legacy, callback) {
    ...
    if (hooks != null ? hooks.length : void 0) {
  ...
  TransactionRunner.prototype.configureTransaction = function(transaction) {
    var configuration, configuredTransaction, expected, fullPath, header, headerKey, headerValue, headers, i, len, mediaType, name, origin, ref, ref1, request, response, skip, splitIndex, status, system;
    ...
    if (indexOf.call((function() {
      ...
      ref1 = Object.keys(headers);
      results1 = [];
      for (i = 0, len = ref1.length; i < len; i++) {
        name = ref1[i];
        results1.push(name.toLowerCase());
      }
      return results1;
    })(), 'user-agent') < 0) {
    ...
    expected = {
      headers: headersArrayToObject(response.headers)
    };
    if (response.body) {
    ...
    return gavel.isValid(transaction.real, transaction.expected, 'response', (function(_this) {
      return function(isValidError, isValid) {
        ...
        return gavel.validate(transaction.real, transaction.expected, 'response', function(validateError, gavelResult) {
          var gavelError, i, len, message, rawValidatorOutput, ref, ref1, ref2, ref3, ref4, ref5, results, sectionName, validatorOutput;
          if (!isValidError && validateError) {
            logger.debug('Gavel.js validation errored:', validateError);
            _this.emitError(validateError, test);
          }
          if ((((ref = (ref1 = test.expected.statusCode) != null ? ref1.toString() : void 0) === '204' || ref === '205') || ((ref2 = (ref3 = test.actual.statusCode) != null ? ref3.toString() : void 0) === '204' || ref2 === '205')) && (test.expected.body || test.actual.body)) {
            logger.warn(test.title + " HTTP 204 and 205 responses must not include a message body: https://tools.ietf.org/html/rfc7231#section-6.3");
          }
          message = '';
          ref4 = gavelResult || {};
          for (sectionName in ref4) {
            if (!hasProp.call(ref4, sectionName)) continue;
            validatorOutput = ref4[sectionName];
            if (sectionName !== 'version') {
              ref5 = validatorOutput.results || [];
  ...
  TransactionRunner.prototype.isMultipart = function(headers) {
    var contentType;
  ...
  TransactionRunner.prototype.fixApiBlueprintMultipartBody = function(body) {
