...
var EventEmitter, HooksWorkerClient, generateUuid, getGoBin, logger, net, path, spawn, spawnArgs, which,
...
HooksWorkerClient = (function() {
  ...
  HooksWorkerClient.prototype.setCommandAndCheckForExecutables = function(callback) {
    ...
    if (this.language === 'ruby') {
      ...
      return getGoBin((function(_this) {
        return function(err, goBin) {
          if (err) {
            return callback(new Error("Go doesn't seem to be installed: " + err.message));
          } else {
            _this.handlerCommand = path.join(goBin, 'goodman');
            _this.handlerCommandArgs = [];
            if (which.which(_this.handlerCommand)) {
              return callback();
            } else {
              msg = "Go hooks handler command not found: " + _this.handlerCommand + "\nInstall go hooks handler by running:\n$ go get github.com/snikch/goodman/cmd/goodman";
              return callback(new Error(msg));
            }
          }
        };
