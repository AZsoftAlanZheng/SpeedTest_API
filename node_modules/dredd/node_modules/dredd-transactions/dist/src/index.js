...
var compileFromApiElements = require('./compile');
...
function compile(source, filename, callback) {
  ...
  parse(source, function (err, parseResult) {
    ...
    var compilationResult = void 0;
    if (!(parseResult ? parseResult.apiElements : undefined)) {
      if (err) {
        return callback(null, createParserErrorCompilationResult(err.message));
      }

      var message = 'The API description parser was unable to provide a valid parse result';
    ...
    // in any case.
    try {
      var mediaType = parseResult.mediaType,
          apiElements = parseResult.apiElements;
