...
readarray array17citis < ./city_list.txt
...
do
  ...
  cityName=`echo $city | cut -f 2 -d -` ## "云南"->"云南", "云南-丽江"->"丽江"
  
  echo $city
  for city17 in "${array17citis[@]}"
  do
    if [ "$useProvince" = true ]; then
      city17Code=`echo $city17 | cut -f 1 -d ' '`
      city17Name=`echo $city17 | cut -f 2 -d ' '`
	else
      city17Code=`echo $city17 | cut -f 3 -d ' '`
      city17Name=`echo $city17 | cut -f 4 -d ' '`
	fi
